...
using TrustEDU.VM.Base.Helpers;
...
namespace TrustEDU.VM.Runtime
{
    public class ExecutionEngine : IDisposable
    {
        ...
        private readonly Dictionary<byte[], HashSet<uint>> breakPoints = new Dictionary<byte[], HashSet<uint>>(new HashComparer());

        public IScriptContainer ScriptContainer { get; }
        public ICryptography Crypto { get; }
        ...
        public void AddBreakPoint(byte[] scriptHash, uint position)
        {
            if (!breakPoints.TryGetValue(scriptHash, out HashSet<uint> hashset))
            {
                hashset = new HashSet<uint>();
                breakPoints.Add(scriptHash, hashset);
            }
                ...
                switch (opcode)
                {
                    ...
                    case OpCode.RET:
                        using (ExecutionContext contextPop = InvocationStack.Pop())
                        {
                            ...
                            if (rvcount == -1) rvcount = contextPop.EvaluationStack.Count;
                            if (rvcount > 0)
                            {
        ...
        public bool RemoveBreakPoint(byte[] scriptHash, uint position)
        {
            if (!breakPoints.TryGetValue(scriptHash, out HashSet<uint> hashset))
                return false;
